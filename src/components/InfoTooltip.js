...
function InfoTooltip(props) {
  ...
  const message = props.regStatus ? successMessage : failMessage
  const icon = props.regStatus ? success : fail
  ...
  return (
      <div className={`popup_type_infoTooltip popup ${props.isOpen && 'popup_opened'}`}
        ...
        <div className="popup-form popup-form_type_infoTooltip">
          <img src={icon} className="popup-form__status-image" alt="статус"/>
          <p className="popup-form__status-message">{message}</p>
          <button type="button"
